...
assert.match(server, /tokenPreviaExclusao\(sessao\.stateJson, cnpjLimpo, exclusao\.resumo\.filtros\) !== body\.previewToken/);
assert.match(server, /montarPreviaExclusao\(state\.entries, req\.body\.filtros \|\| req\.body\)/);
assert.match(server, /aplicarExclusao\(state\.entries, body\.filtros \|\| body, chavesSelecionadas\)/);
...
assert.match(adminHtml, /id="exclusaoTipoMovimento"/);
assert.match(adminHtml, /id="exclusaoLancamentoInicial"/);
assert.match(adminHtml, /id="exclusaoLancamentoFinal"/);
assert.match(adminHtml, /id="exclusaoConta"/);
assert.match(adminHtml, /id="exclusaoHistorico"/);
assert.match(adminHtml, /id="exclusaoValor"/);
assert.match(adminHtml, /filtros: previa\.filtros/);
