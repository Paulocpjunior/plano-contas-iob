...
const custom = carregarRuntimeConfig({
  ...
  CCI_DATA_DATABASE_ID: 'cci-homologacao-123',
...
assert.throws(() => carregarRuntimeConfig({ CCI_DATA_DATABASE_ID: '(outro)' }), /CCI_DATA_DATABASE_ID/);
assert.strictEqual(carregarRuntimeConfig({ CCI_DATA_DATABASE_ID: 'cci-restore-test-20260829' }).dataDatabaseId, 'cci-restore-test-20260829');

const server = fs.readFileSync(require.resolve('../server'), 'utf8');
assert.ok(server.includes('databaseId: runtimeConfig.dataDatabaseId'));
...
for (const nome of ['CCI_RUNTIME_PROJECT_ID', 'CCI_DATA_PROJECT_ID', 'CCI_DATA_DATABASE_ID', 'CCI_AUTH_PROJECT_ID', 'CCI_BACKUP_PROJECT_ID', 'CCI_BACKUP_BUCKET']) {
