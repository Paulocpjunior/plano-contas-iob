...
assertContains('server.js', 'MEMORIA_EXISTENTE_DIVERGENTE', 'sobrescrita divergente de memoria restrita a administrador');
assertContains('server.js', 'mesmaClassificacaoAprendida', 'repeticao identica de memoria tratada como idempotente');
...
assertContains('index.html', 'layout FLANACAR Registro Fiscal detectado antes do CSV generico', 'parser FLANACAR deve rodar antes da rejeicao CSV generica');
...
assertContains('index.html', 'FLANACAR - REGISTROS FISCAIS CSV (ENTRADAS/SAÍDAS)', 'campo Banco exibe opcao FLANACAR fiscal para ambos os livros');
assertContains('index.html', 'parsearCSV_FlanacarRegistroSaidas', 'fluxo principal carrega parser exclusivo do livro de saidas');
...
assertContains('index.html', 'cnpjCliente', 'IA recebe o CNPJ do cliente nas saidas FLANACAR');
assertContains('index.html', 'detectarBancoFiscalFlanacarNoPreview', 'preview reconhece CSV de movimento fiscal antes do parser generico');
assertContains('index.html', 'CSV - FLANACAR Registro de Entradas Fiscal', 'preview identifica visualmente CSV FLANACAR fiscal');
assertContains('index.html', 'CSV - FLANACAR Registro de Saídas Fiscal', 'preview identifica visualmente o livro de saidas FLANACAR');
assertContains('index.html', 'Selecionar colunas do livro fiscal', 'FLANACAR deve abrir selecao de colunas apos validar o arquivo');
...
assertContains('index.html', 'Movimento Fiscal — Livro de ', 'preview deve confirmar visualmente a direcao do movimento fiscal reconhecido');
assertContains('index.html', 'Abrir importações fiscais', 'arquivo fiscal no extrator generico deve encaminhar para o modal seguro');
assertContains('index.html', 'atualizarIndicadorFlanacar(true, fiscalMeta.direcao)', 'indicador de colunas deve refletir entradas ou saidas somente apos detectar FLANACAR');
assertContains('index.html', 'A importação deve ser feita pelo modal fiscal com trava de CNPJ', 'movimento fiscal nao pode ser gravado pelo extrator bancario generico');
assertContains('index.html', 'validarVinculoCnpjFiscal', 'modal fiscal deve validar CNPJ antes de gravar');
assertContains('server.js', 'parser-flanacar-registro-entradas\\.js', 'parser FLANACAR deve ser servido sem cache antigo');
...
assertContains('auditai/index.html', `rol-core.js?v=${version}`, 'cache-buster atual do calculo ROL AuditAI');
assertContains('auditai/index.html', `rol-reports.js?v=${version}`, 'cache-buster atual dos relatorios ROL AuditAI');
...
assertContains('auditai/rol-core.js', 'calculateAnalysis', 'AuditAI possui calculo ROL individual testavel');
assertContains('auditai/rol-core.js', 'validateGroup', 'AuditAI valida CNPJ e periodo do grupo antes da ROL');
assertContains('auditai/rol-reports.js', 'exportIndividualPdf', 'AuditAI exporta ROL individual em PDF');
assertContains('auditai/rol-reports.js', 'exportIndividualXlsx', 'AuditAI exporta memoria individual ROL em Excel');
assertContains('auditai/rol-reports.js', 'hasValidIndividualCnpj', 'AuditAI exige CNPJ valido no relatorio ROL individual');
...
assertContains('auditai/rol-reports.js', 'exportAnalysisExecutiveBig4Pdf', 'AuditAI oferece relatorio executivo BIG4 geral fora da ROL');
...
assertContains('auditai/assets/index-DREfix3266.js', 'OFFICIAL_RECEITA_OPERACIONAL_BRUTA', 'AuditAI solicita e preserva receita operacional bruta oficial');
assertContains('auditai/assets/index-DREfix3266.js', 'OFFICIAL_DEDUCOES_RECEITA', 'AuditAI solicita e preserva deducoes oficiais da receita');
assertContains('auditai/assets/index-DREfix3266.js', 'OFFICIAL_RECEITA_OPERACIONAL_LIQUIDA', 'AuditAI solicita e preserva ROL oficial');
...
assertContains('auditai/assets/index-DREfix3266.js', 'auditaiExecutiveBig4Pdf', 'AuditAI exibe BIG4 no cabecalho da analise individual');
assertContains('auditai/assets/index-DREfix3266.js', '"Exportar PDF"]})', 'AuditAI preserva o PDF individual existente');
