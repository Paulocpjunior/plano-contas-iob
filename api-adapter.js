(function () {
  ...
  // R-2055 (aquisição de produção rural) — apura no CFI e GERA o XML do evtAqProd
  // para importar no IOB (que transmite). O app não transmite: padrão IOB, sem
  // risco de dupla transmissão.
  async function reinfGerarXmlAquisicaoRural(dados) {
    const r = await apiFetch(API_BASE + '/api/reinf/aquisicao-rural/gerar-xml', { method: 'POST', body: JSON.stringify(dados || {}) });
    return await r.json();
  }
