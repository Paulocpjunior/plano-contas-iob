(function () {
  ...
  async function processarAdminAlertasProgressao(competencia, cnpj) {
    const r = await apiFetch(API_BASE + '/api/admin/progressao-contabil/processar-alertas', {
      method: 'POST', body: JSON.stringify({ competencia: competencia, cnpj: cnpj || '' })
    });
    const body = await r.json().catch(() => ({}));
    if (!r.ok) throw new Error(body.erro || ('Erro ' + r.status));
    return body;
  ...
  async function avaliarConciliacaoDetalhada(cnpj, dados) {
    const cnpjLimpo = String(cnpj || '').replace(/\D/g, '');
    const r = await apiFetch(API_BASE + '/api/empresas/' + cnpjLimpo + '/contabilidade/conciliacoes/movimentos/avaliar', { method: 'POST', body: JSON.stringify(dados || {}) });
    const body = await r.json().catch(() => ({}));
    if (!r.ok) throw new Error(body.erro || (body.erros || []).join(' ') || ('Erro ' + r.status));
    return body;
  }

  async function aprovarConciliacaoDetalhada(cnpj, dados) {
    const cnpjLimpo = String(cnpj || '').replace(/\D/g, '');
    const r = await apiFetch(API_BASE + '/api/empresas/' + cnpjLimpo + '/contabilidade/conciliacoes/movimentos/aprovar', { method: 'POST', body: JSON.stringify(dados || {}) });
    const body = await r.json().catch(() => ({}));
    if (!r.ok) throw new Error(body.erro || ('Erro ' + r.status));
    return body;
  }
  ...
  // ✍️ AJUSTE da retenção de UMA nota (31/08). O valor é gravado no CFI, que é
  // o dono do dado — ajuste guardado aqui faria o SPED e a EFD-Reinf declararem
  // números diferentes sobre a mesma nota.
  async function reinfAjustarRetencao(cnpj, competencia, corpo) {
    const c = String(cnpj || '').replace(/\D/g, '');
    const r = await apiFetch(
      API_BASE + '/api/reinf/retencoes-pj/' + c + '/' + encodeURIComponent(competencia || '') + '/ajuste',
