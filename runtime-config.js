...
function bancoFirestore(valor, nome) {
  const databaseId = String(valor || '').trim();
  if (databaseId !== '(default)' && !/^[a-z][a-z0-9-]{2,61}[a-z0-9]$/.test(databaseId)) {
    throw new Error(`Configuração inválida em ${nome}: informe (default) ou um database ID explícito.`);
  }
  return databaseId;
}
...
function carregarRuntimeConfig(env) {
  ...
  const dataDatabaseId = bancoFirestore(origem.CCI_DATA_DATABASE_ID || DEFAULTS.dataDatabaseId, 'CCI_DATA_DATABASE_ID');
  ...
  return Object.freeze({ runtimeProjectId, dataProjectId, dataDatabaseId, authProjectId, backupProjectId, backupBucket });
}
...
function identidadePublica(config) {
  return Object.freeze({
    ...
    database: config.dataDatabaseId,
