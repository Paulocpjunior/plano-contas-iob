...
1. Abra **Conferir recebimentos do proprietário**. O rateio sugerido usa o total efetivamente recebido, inclusive IPTU. Informe as exclusões comprovadas atribuíveis ao proprietário e justifique-as. O IPTU incluído no total e a taxa de administração identificada no cabeçalho da planilha são sugeridos em campos separados. A taxa incide sobre o recebido sem IPTU. Confirme os comprovantes; se não houver percentual identificado, informe o valor da taxa (inclusive zero, se não existir). O campo Base após exclusões mostra o resultado, sem repetir as exclusões nas deduções mensais.
